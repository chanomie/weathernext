...
public class WeatherApiController {
    ...
    */
	...
	public @ResponseBody Map<String,String> getGoogleStatus(
       ...
       if(principal != null) {
	       ...
	       urlMap.put("recipientEmail", userService.getCurrentUser().getEmail());
    ...
    */
	...
	public @ResponseBody Map<String,String> getGoogleStatus(
		...
		responseJson.put("recipientEmail", userService.getCurrentUser().getEmail());
		responseJson.put("ownerId", userService.getCurrentUser().getUserId());
		responseJson.put("nickname", userService.getCurrentUser().getNickname());
	...
	public @ResponseBody List<Map<String,String>> getSchedules(
		...
		log.finer("Getting schedule for recipientEmail ["+recipientEmail+"], ownerId ["+ownerId+"]" );
		
		if(recipientEmail != null && zipcode != null) {
			WeatherEmailSchedule weatherEmailSchedule = 
					weatherEmailScheduleHelper.getWeatherEmailSchedule(ownerId, recipientEmail, zipcode);
			...
			List<WeatherEmailSchedule> weatherScheduleList = 
					weatherEmailScheduleHelper.getWeatherEmailSchedule(ownerId, recipientEmail);
	...
	public @ResponseBody Map<String,String> assignSchedule(
			@RequestParam(value="scheduleKey", required=false) String scheduleKey,
			@RequestParam(value="ownerId", required=false) String ownerId,
			Principal principal
		) {
		
		if(principal == null) throw new SecurityException("Requires login");
		if(! userService.isUserAdmin()) throw new SecurityException("Requires admin user");
		
		WeatherEmailSchedule weatherEmailSchedule = 
				weatherEmailScheduleHelper.assignSchedule(scheduleKey, ownerId);
	...
	public @ResponseBody List<Map<String,String>> deleteSchedulePost(
			@RequestParam(value="scheduleKey", required=false) String scheduleKey,
			Principal principal
		) {
	...
	public @ResponseBody Map<String,String> deleteSchedule(
		...
		WeatherEmailSchedule weatherEmailSchedule =
				weatherEmailScheduleHelper.deleteWeatherEmailSchedule(recipientEmail, scheduleKey);
	...
	public @ResponseBody Map<String,String> addSchedule(
		...
		log.finer("Got ownerId [" + ownerId + "], recipientName [" + recipientName
				+ "], recipientEmail [" + recipientEmail + "], zip ["
				+ zipcode + "], timezone [" + timezoneString +"], sendTime ["
				+ sendTimeString + "]");
		...
		WeatherEmailSchedule weatherEmailSchedule = 
				weatherEmailScheduleHelper.putSchedule(ownerId, recipientName, recipientEmail, zipcode, timezone, sendTime);
