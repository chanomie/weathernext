 ...
 * Zip code file comes from: http://notebook.gaslampmedia.com/wp-content/uploads/2013/08/zip_codes_states.csv
 * @author jreed
 *
 */
public class ZipCodeLookup {
	Map<String,LocationData> zipCodeData = new HashMap<String,LocationData>();
	private final static Logger logger = Logger.getLogger(ZipCodeLookup.class.getName()); 
	...
	public ZipCodeLookup() {
		...
		Reader zipCodeListReader = new BufferedReader(new InputStreamReader(this.getClass().getResourceAsStream("/zipcodes/zipcodes.csv")));
	...
	public GeoCoord getGeoCoord(String zipcode) {
		...
		if(locationData != null) {
			try {
				result = new GeoCoord(locationData.getLongitude(), locationData.getLatitude());
			} catch (NumberFormatException e) {
				logger.log(Level.WARNING, "Failed to parse coordinates for zip [" + zipcode + "]", e);
				// Couldn't make it.
			}
		}
